...
In figure~\ref{overhead-of-monitoring-with_heapexplorer} we compare the execution time of the same tests but using different memory monitoring approaches.
...
% legend
\legend{Memory instrumentation, Heap Exploration, Memory \& Instructions instrumentation, Heap Exploration \& Instructions Instrumentation}
\end{axis}
\end{tikzpicture}
...
\begin{axis}[
every axis legend/.append style={nodes={right}},
ybar=0pt, legend style={at={(0.8,1)},
anchor=north,legend columns=1, font=\tiny},
ylabel={Time (seconds)},
y label style={at={(0.04, 0.5)}},
scaled y ticks = false,
...
\legend{
	...
	Adaptive with All Components and Heap Exploration, 
	%Localized monitoring, 
	Localized monitoring with Heap Exploration,
	Global Monitoring}
\end{axis}
\end{tikzpicture}
