...
In particular, the identified limitations are related to three dimensions we can use to assess the overall quality of a mechanism: many techniques still require considerable knowledge about the target technology, tools built using existent approaches often induce high performance overhead, and many times existent approaches lack the power to express how to build efficient tools.
Unsurprisingly, these three dimensions are often in contradictions.
For instance, many approaches that offer good performance overhead require considerable knowledge about the target technology.
The question is whether the trade-off followed by different approaches are good enough.
In this section, we discuss in details the limitations we find in existent approaches.  

\begin{description}
\item[Often knowledge of low-level details is required] need knowledge about the target technology (hace que no sea tan facil como se supone crear tools)
\item[Tools built have high overhead] performance (hace que no se puedan usar en ambientes de produccion)
\item muchas herramientas tienen limites en su poder, es decir en lo que pueden expresar, esto es particularmente cierto en aspects.
Tambien hay que considerar que algunas cosas se pueden expresar pero, producto del modelo de ejecucion de estos mecanismo, con un alto costo en rendimiento.
\end{description}
