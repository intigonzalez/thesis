...
\todo{worst title ever}

\begin{itemize}
\item \textit{Brief introduction.}
\item \textit{Usage in MRTEs.}
\item \textit{Resource consumption.}
\item \textit{Example of differences among concrete abstractions.}
\item \textit{Specific features to consider when resource awareness support is implemented.}
\item \textit{State of the art on providing resource awareness support for component-based systems.}
\end{itemize}
...
It is straightforward to identify in the code when a component is requesting a service because a port is used to do so. 
As a consequence, an automated tool can easily instrument the code to detect when components are communicating.
On the contrary, OSGi uses a plain Java Interfaces and objects to connect bundles.
In this cases it is more complex to detect this communication because there is not a single interface used.
Instead, a bundle can communicate with several services using different interfaces. 
...
\subsection{Domain-Specific Languages and MRTEs} \label{sec:DSL-on-MRTEs}
